...
def cal_memorization_auc(dataname, generated_path, train_data, thresholds=None):

    if thresholds is None:
        thresholds = np.linspace(0, 1, 10)  # Default range of thresholds from 0 to 1
    ...
    for threshold in thresholds:
        ...
        for index, W in generated_data.iterrows():
            ...
            if ratio < threshold:  # Compare with the current threshold
                replicate_count += 1

        replicate_ratio = replicate_count / len(generated_data)
        threshold_replicate_ratios.append(replicate_ratio)

    # Calculate AUC
    auc = np.trapz(threshold_replicate_ratios, thresholds)

    print(f"{dataname.capitalize()} - Area under the curve (AUC): {auc:.4f}")
    return threshold_replicate_ratios, thresholds, auc
...
def main():
    ...
    generated_path = f'TabSMOTE/TabSMOTE/synthetic_82/{dataset}/{model}.csv'
    train_data_path = f'TabSMOTE/TabSMOTE/synthetic_82/{dataset}/real.csv'
    ...
    if dataset == 'magic':
        train_data.columns = ['Length', 'Width', 'Size', 'Conc', 'Conc1',
    ...
    try:
        cal_memorization_auc(dataset, generated_path, train_data)
